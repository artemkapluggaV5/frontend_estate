...
const Dashboard: React.FC = () => {
  ...
  const fetchRequests = async () => {
    try {
      ...
      if (role === 'admin') {
        const callsRes = await axios.get(`${API_BASE}/api/call-requests/`, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const updateCallStatus = async (id: number, status: string) => {
    try {
      await axios.patch(`${API_BASE}/api/call-requests/${id}/`, { status }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      toast.success('Статус заявки на звонок обновлен');
      fetchRequests();
    } catch (error) {
  ...
  return (
    <div className="container">
      ...
      <div className="dashboard-content">
        ...
        {role === 'admin' && (
          <div className="card requests-card" style={{ marginTop: '2rem' }}>
            <h2 className="requests-title">Заказы звонков</h2>
            
            {loading ? (
              <p>Загрузка...</p>
            ) : callRequests.length === 0 ? (
              <p className="empty-requests">Нет заявок на звонки.</p>
            ) : (
              <div className="requests-list">
                {callRequests.map((call: any) => (
                  <div key={call.id} className="request-item" style={{ borderLeft: call.status === 'new' ? '4px solid var(--danger)' : '4px solid var(--success)' }}>
                    <div>
                      <h4 className="request-title" style={{ color: 'var(--text)' }}>
                        {call.name}
                      </h4>
                      <p className="request-meta">
                        <i className="pi pi-phone" style={{marginRight: '6px', color: 'var(--text-light)'}}></i>
                        <a href={`tel:${call.phone}`} style={{fontWeight: 600, color: 'var(--primary)'}}>{call.phone}</a>
                      </p>
                      <p className="request-meta">
                        Дата создания: {new Date(call.created_at).toLocaleString('ru-RU')}
                      </p>
                      {call.time_to_call && (
                        <p className="request-scheduled">
                          <i className="pi pi-clock" style={{marginRight: '6px'}}></i> Удобное время: {call.time_to_call}
                        </p>
                      )}
                    </div>
                    <div className="request-actions">
                      {call.status === 'new' ? (
                        <span style={{ display: 'inline-block', padding: '4px 12px', borderRadius: '20px', fontSize: '0.75rem', fontWeight: 600, color: 'var(--danger)', backgroundColor: 'rgba(239, 68, 68, 0.1)' }}>Новая</span>
                      ) : (
                        <span style={{ display: 'inline-block', padding: '4px 12px', borderRadius: '20px', fontSize: '0.75rem', fontWeight: 600, color: 'var(--success)', backgroundColor: 'rgba(16, 185, 129, 0.1)' }}>Обработана</span>
                      )}
                      
                      {call.status === 'new' && (
                        <button onClick={() => updateCallStatus(call.id, 'processed')} className="btn btn-primary action-btn" style={{ marginTop: '0.5rem' }}>
